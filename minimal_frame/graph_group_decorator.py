...
def undirected_weighted_sn_equivariant_decorator(forward_func):
    ...
    def wrapper(x, *args, **kwargs):
        assert x.dim() == 2 and x == x.transpose(0, 1)
        n, _ = x.size()
        node_attr = x.diag()
        edge_index = (x != 0).nonzero(as_tuple=False).t()
        edge_attr = x[x != 0]
        frames = generate_permutation_frames(node_attr.detach().cpu().numpy(), edge_index.detach().cpu().numpy(), edge_attr.detach().cpu().numpy())
