...
class FrameAveraging(Transform):
    ...
    def __init__(self, frame_averaging=None, fa_method=None, trainable=False):
    ...
    def __call__(self, data):
        if self.inactive:
            ...
            data.fa_pos, data.fa_cell, data.fa_rot = self.fa_func(
                data.pos, data.cell if hasattr(data, "cell") else None, self.fa_method, self.trainable
            )
            return data
...
def get_transforms(trainer_config, trainable=False):
    transforms = [
        AddAttributes(),
        GraphRewiring(trainer_config.get("graph_rewiring")),
        FrameAveraging(trainer_config["frame_averaging"], trainer_config["fa_method"], trainable),
    ]
